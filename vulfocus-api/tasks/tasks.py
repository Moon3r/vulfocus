    ...
    if user_info.is_superuser:
        ...
        for services_name in args_yaml['services']:
            if "image" in args_yaml['services'][services_name]:
                ...
                all_arg = {"img_name": image_name, "images_name": image_info.image_name,
                           'args_yaml': json.dumps(args_yaml), 'tag': tag}
                if image_name:
    ...
    if user_info.is_superuser or user_id == container_vul.user_id:
        ...
        if countdown == 0:
            ...
            if 'del_container' in setting_config:
                ...
                if not del_container or del_container == 0 or del_container == '0':
                    add_chain_sig = chain(
                        run_docker_compose.s(image_id, container_vul.container_id, user_id, time_model_id, task_id,
                                             countdown, ) |
            ...
            try:
                ...
                with transaction.atomic():
                    for container in container_list:
                        ...
                        container_vul_img = ContainerVul(image_id=img_info, user_id=user_info.id,
                                                         vul_host=container_host,
...
def run_container(container_id, user_id, tmp_task_id, countdown):
    ...
    subdomain = ""
    try:
        frpx = FrpUtils.objects.filter(container_id=container_vul.container_id).first()
        subdomain = frpx.subdomain
    except:
        pass

    switchon = FrpConfig.objects.get(frp_key="switch").frp_value
    ...
    if not docker_container:
        ...
        try:
            ...
            image_info.save()
            return str(task_info.task_id)
        vul_flag = "flag{%s}" % (str(uuid.uuid4()).replace("-", ""),)
        if container_vul.container_flag:
            vul_flag = container_vul.container_flag
        try:
            if switchon == "true":
                docker_container = client.containers.run(image_name, detach=True,
                                                         environment={"vul_flag": vul_flag},
                                                         network=frp_config.get("frp_network"))
            else:
            ...
            image_info.save()
            return str(task_info.task_id)
        command = 'sh -c "echo "%s" >/flag"' % (vul_flag,)
        if switchon == "true":
            subdomain = str(uuid.uuid4()).replace("-", "")
            vul_host = "%s.%s" % (subdomain, frp_config.get("domain"), )
        else:
    ...
    if "running" == docker_container.status:
        ...
        task_id = str(task_info.task_id)
    if switchon == "true":
        frp_start.delay(container_vul.container_id, container_vul.docker_container_id, port_list[0], subdomain)
...
def update_images():
    ...
    for item_num in range(int(num) + 1):
        new_url = "https://hub.docker.com/v2/repositories/vulfocus/?ordering=last_updated&page={}&page_size=100".format(
            item_num + 1)
...
@shared_task(name="tasks.frp_start")
def frp_start(container_id, docker_container_id, container_port, subdomain, http=True):
    http_template = "\n\n[http_%s]\n" + \
                    "type = http\n" + \
                    "local_ip = %s\n" + \
                    "local_port = %s\n" + \
    ...
    output = get_frp_now(container_id)
    did = docker_container_id[:12]
    config = ""
    if http:
        config += http_template % (did, did, container_port, subdomain)
    else:
        config += direct_template % (did, did, container_port, container_port,
                                     did, did, container_port, container_port)

    container_go = ContainerVul.objects.get(container_id=container_id)
    if not container_go:
        return R.err(data={"status": "error"}, msg="容器未运行")
    else:
        if container_go.container_status != "running":
            return R.err(data={"status": "error"}, msg="容器未运行")
        frp_search = FrpUtils.objects.filter(container_id=container_id).first()
        if not frp_search:
            frp_save = FrpUtils(container_id=container_id, subdomain=subdomain, frp_config=config)
            frp_save.save()
        else:
            if frp_search.frp_config != config:
                frp_search.frp_config = config
            if frp_search.subdomain != subdomain:
                frp_search.subdomain = subdomain
            frp_search.save()
        output += config
    data = frp_util(output)
    return data


@shared_task(name="tasks.frp_stop")
def frp_stop(container_id):
    output = get_frp_now(container_id, True)
    try:
        frp = FrpUtils.objects.get(container_id=container_id)
        containervul = ContainerVul.objects.filter(container_id=container_id).first()
        if containervul.container_status not in ["running", "stop"]:
            frp.delete()
    except:
        frp.delete()
    data = frp_util(output)
...
def frp_util(output):
    ...
    configs = get_frp_configs()
    frp_upload_api = "http://%s:7400/api/config" % (configs.get("frp_url"),)
    frp_reload_api = "http://%s:7400/api/reload" % (configs.get("frp_url"),)

    res = requests.put(url=frp_upload_api, data=output)
    data = {
        "status": res.status_code
    }
    if res.ok:
        res = requests.get(url=frp_reload_api)
        data = {
            "status": res.status_code
        }
        if res.ok:
            ...
            return R.err(data=data, msg="Frp重启失败")
    else:
        return R.err(data=data, msg="Frp配置文件异常")


def get_frp_now(cid=None, stop=False):
    frp_utils = FrpUtils.objects.all()
    configs = get_frp_configs()
    output = configs.get("frp_template")
    for frp in frp_utils:
        if frp.container_id == cid and stop:
            continue
        try:
            container = ContainerVul.objects.get(container_id=frp.container_id)
            if container.container_status == 'running' and cid != frp.container_id:
                output += frp.frp_config
        except:
            continue
...
def get_frp_configs():
    frps = FrpConfig.objects.all()
    configs = {}
    for frp in frps:
        configs[frp.frp_key] = frp.frp_value
...
def create_layout_image_download_task(layout_info, user_info):
    ...
    for service in services:
        ...
        task_info = TaskInfo(user_id=user_info.id, task_name="环境编排相关镜像下载:" + image_name, task_status=1,
                             task_start_date=timezone.now(), operation_type=8,
